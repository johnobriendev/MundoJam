...
export default function PastJamCard({ occurrence }: { occurrence: JamOccurrenceWithJam }) {
  const { jam } = occurrence

  return (
    <Link
      href={`/jams/${occurrence.id}`}
      className="block rounded-lg border bg-surface p-4 hover:border-[var(--border-focus)] hover:shadow-sm transition-all opacity-70 hover:opacity-90"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h3 className="font-semibold text-secondary truncate">{jam.title}</h3>
          <p className="text-sm text-secondary mt-0.5 opacity-75">
      ...
      {jam.genres.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1">
          {jam.genres.map((g) => (
            <span
              key={g.genre}
              className="text-xs bg-muted text-secondary rounded-full px-2 py-0.5"
            >
              {g.genre}
            </span>
          ))}
